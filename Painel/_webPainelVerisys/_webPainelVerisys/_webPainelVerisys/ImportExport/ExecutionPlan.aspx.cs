    ...
    public partial class ExecutionPlan : System.Web.UI.Page
    {
        ...
        protected void gdRegistros_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Upd")
            {
                ...
                if (!String.IsNullOrEmpty(gdRegistros.DataKeys[index]["StartDateTime"].ToString()))
                {
                    DateTime StartDateTime = DateTime.Parse(gdRegistros.DataKeys[index]["StartDateTime"].ToString());
                    string strStartDate = StartDateTime.ToString("dd/MM/yyyy");
                    string strStartTime = StartDateTime.ToString("HH:mm:ss");
                    txtStartTime.Text = strStartTime;
                    txtStartDate.Text = strStartDate;

                }
                if (!String.IsNullOrEmpty(gdRegistros.DataKeys[index]["EndDateTime"].ToString()))
                {
                    DateTime EndDateTime = DateTime.Parse(gdRegistros.DataKeys[index]["EndDateTime"].ToString());
                    string strEndDate = EndDateTime.ToString("dd/MM/yyyy");
        ...
        protected void Cadastro()
        {
            ...
            try
            {
                ...
                if (txtStartTime.Text != String.Empty && txtStartDate.Text != String.Empty)
                    dtoAux.StartDateTime = Convert.ToDateTime(Convert.ToDateTime(txtStartDate.Text + " " + txtStartTime.Text).ToString("yyyy-MM-dd HH:mm:ss"));


                if (txtEndTime.Text != String.Empty && txtEndDate.Text != String.Empty)
                    dtoAux.EndDateTime = Convert.ToDateTime(Convert.ToDateTime(txtEndDate.Text+" "+txtEndTime.Text).ToString("yyyy-MM-dd HH:mm:ss"));
